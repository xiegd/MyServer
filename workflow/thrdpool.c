...
static void __thrdpool_terminate(int in_pool, thrdpool_t *pool) {
  ...
  if (in_pool) {
    ...
    pthread_detach(
        pthread_self());  // 将当前线程从线程池中分离，避免其资源被回收
    pool->nthreads--;
  }
  // 等待所有线程完成
  ...
  pthread_mutex_unlock(&pool->mutex);
  if (!pthread_equal(pool->tid, __zero_tid)) pthread_join(pool->tid, NULL);
}
...
static int __thrdpool_create_threads(size_t nthreads, thrdpool_t *pool) {
  ...
  if (ret == 0) {
    ...
    if (pool->nthreads == nthreads) return 0;  // 创建成功返回0
...
thrdpool_t *thrdpool_create(size_t nthreads, size_t stacksize) {
  ...
  pool = (thrdpool_t *)malloc(
      sizeof(thrdpool_t));  // 分配内存空间， 分配失败返回NULL
  if (!pool) return NULL;   // 创建失败返回NULL

  pool->msgqueue = msgqueue_create(
      0, 0);  // 创建消息队列，用于接收和存储任务, 创建失败返回NULL
  if (pool->msgqueue) {
    ret = pthread_mutex_init(&pool->mutex,
                             NULL);  // 初始化互斥锁， 成功返回0，失败返回errno
    if (ret == 0) {
      ret = pthread_key_create(&pool->key,
                               NULL);  // 创建TSD key, 成功返回0， 失败返回errno
      if (ret == 0) {
        // 如果线程池内存分配成功，msgqueue创建成功，mutex初始化成功,
        // TSD key创建成功
...
int thrdpool_decrease(thrdpool_t *pool) {
  ...
  if (buf) {
    ...
    msgqueue_put_head(
        entry,
        pool->msgqueue);  // 将任务放入线程池的消息队列队头，触发线程退出
    return 0;
  }
