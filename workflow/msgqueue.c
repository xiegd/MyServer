 ...
 *
 * 使用双链表加锁的机制实现线程安全的消息传递，两个链表get_list,
 * put_list分别处理消息 的获取和放入操作。
...
struct __msgqueue {
  ...
  void *head1;   // get_list
  void *head2;   //  put_list,
                //  一个指向链表头，一个指向链表尾，从而实现队列的尾插和头插
  void **get_head;  // 指向get_list的头部指针， 头部也是一个指针，指向msg结构体
  void **put_head;  // 指向put_list的头部指针
  void **
      put_tail;  // 指向put_list的尾部指针,
                 // 指向最后一条消息的指针域?，put_head和put_tail是指向同一个地方的
...
void msgqueue_put(void *msg, msgqueue_t *queue) {
  void **link =
      (void **)((char *)msg +
                queue->linkoff);  // 获取msg所链接的下一条消息的指针的地址,
                                  // msg包含数据域和指针域
  ...
  *link = NULL;  // 将指针域置为NULL
  ...
  while (queue->msg_cnt > queue->msg_max - 1 && !queue->nonblock)
    pthread_cond_wait(
        &queue->put_cond,
        &queue
             ->put_mutex);  // 释放锁，阻塞等待(条件满足后会自动获取锁，只有获取到锁才能继续执行)
  // 为什么添加的是link?
  *queue->put_tail =
      link;  // 对put_tail进行解引用，获得的是尾部的msg结构体的指针，这里更新了尾部msg结构体的指针域，即添加了一条消息
  queue->put_tail = link;  // 更新put_tail, 使其指向更新后的尾部
  queue->msg_cnt++;
  pthread_mutex_unlock(&queue->put_mutex);
  pthread_cond_signal(
      &queue->get_cond);  // 唤醒等待的线程（等待获取消息的线程）
}
...
void msgqueue_put_head(void *msg, msgqueue_t *queue) {
  ...
  while (*queue->get_head) {
    ...
    if (pthread_mutex_trylock(&queue->get_mutex) == 0) {
      pthread_mutex_unlock(
          &queue->put_mutex);  // 这个释放锁的位置是不是有问题？
  ...
  *link = *queue->put_head;
  if (*link == NULL) queue->put_tail = link;
...
msgqueue_t *msgqueue_create(size_t maxlen, int linkoff) {
  ...
  if (ret == 0) {
    ret = pthread_mutex_init(&queue->put_mutex,
                             NULL);  // 初始化放入操作的互斥锁
    if (ret == 0) {
      ret = pthread_cond_init(&queue->get_cond,
                              NULL);  // 初始化获取操作的条件变量
      if (ret == 0) {
        ret = pthread_cond_init(&queue->put_cond,
                                NULL);  // 初始化放入操作的条件变量
        if (ret == 0) {
          queue->msg_max = maxlen;
          queue->linkoff = linkoff;  // 消息结构体中消息链表指针的偏移，
                                     // 和消息类型有关
          queue->head1 = NULL;
          queue->head2 = NULL;
