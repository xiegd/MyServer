/*
 *   待补充和完善的部分：整合配置文件和命令行参数，可以使用命令行选项对配置项进行覆盖，并标明为
 *   命令行配置；
 *   支持对配置项进行校验;
 */
...
class ConfigVarBase {
 public:
  ...
  ConfigVarBase(const std::string& name, const std::string& description = "")
    ...
    std::transform(
        m_name.begin(), m_name.end(), m_name.begin(),
        ::tolower);  // 将m_name转换为小写形式（STL算法），存放到m_name中
  }
  ...
  virtual bool fromString(
      const std::string&
          val) = 0;  // 从字符串初始化值, 纯虚函数，在派生类中实现
  virtual std::string getTypeName()
      const = 0;  // 返回配置参数值的类型名称, 纯虚函数，在派生类中实现
 ...
 public:
  std::vector<T> operator()(const std::string& v) {
    ...
    for (size_t i = 0; i < node.size(); ++i) {
      ss.str("");
      ss << node
              [i];  // 下面创建了一个LexicalCast类型的临时变量，然后调用重载的`()`运算符
      vec.push_back(LexicalCast<std::string, T>()(
          ss.str()));  // 使用仿函数实现基本类型to string
    }
    return vec;
  }
};
...
class LexicalCast<std::vector<T>, std::string> {
 public:
  std::string operator()(const std::vector<T>& v) {
    ...
    for (auto& i : v) {
      node.push_back(YAML::Load(LexicalCast<T, std::string>()(
          i)));  // 使用前面实现的仿函数，进行基本类型和string之间的转换
    }
    std::stringstream ss;
    ss << node;
    return ss.str();
...
// 配置参数模板子类,保存对应类型的参数值, T 参数的具体类型, std::string
// 为YAML格式的字符串 一个参数对应一个类实例，
template <class T, class FromStr = LexicalCast<std::string, T>,
          class ToStr = LexicalCast<
              T, std::string> >  // 定义三个模板参数，后两个给出默认参数值
class ConfigVar : public ConfigVarBase {
 public:
  typedef RWMutex RWMutexType;
  typedef std::shared_ptr<ConfigVar> ptr;
  typedef std::function<void(const T& old_value, const T& new_value)>
      on_change_cb;  // 定义回调函数, 函数接受两个参数，没有返回值
  ...
  std::string toString() override {
    try {
      ...
      SYLAR_LOG_ERROR(SYLAR_LOG_ROOT())
          ...
          << " name=" << m_name;  // e.what(): 异常信息，TypeToName<T>():
                                  // 将对应类型名转换为str
    }
    return "";
  }
...
class Config {
 public:
  ...
  static typename ConfigVar<T>::ptr Lookup(
    ...
    // 创建配置项, 使用typename明确告诉编译器，这是一个类模板中的类型,
    // ptr是ConfigVar的shared_ptr
    typename ConfigVar<T>::ptr v(new ConfigVar<T>(
        name, default_value, description));  // 使用智能指针的初始化
    GetDatas()[name] = v;
    return v;
  }
