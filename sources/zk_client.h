...
class ZKClient : public std::enable_shared_from_this<ZKClient> {
 public:
  ...
  int32_t create(const std::string& path, const std::string& val,
                 std::string& new_path,
  ...
  int32_t get(const std::string& path, std::string& val, bool watch,
              Stat* stat = nullptr);
  ...
  int32_t set(const std::string& path, const std::string& val, int version = -1,
              Stat* stat = nullptr);
  int32_t getChildren(const std::string& path, std::vector<std::string>& val,
                      bool watch, Stat* stat = nullptr);
  int32_t close();
  int32_t getState();
 ...
 private:
  static void OnWatcher(zhandle_t* zh, int type, int stat, const char* path,
                        void* watcherCtx);
  typedef std::function<void(int type, int stat, const std::string& path)>
      watcher_callback2;
