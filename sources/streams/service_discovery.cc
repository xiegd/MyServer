...
void IServiceDiscovery::listServer(
    std::unordered_map<
        std::string,
        std::unordered_map<
            std::string, std::unordered_map<uint64_t, ServiceItemInfo::ptr> > >&
        infos) {
...
bool ZKServiceDiscovery::existsOrCreate(const std::string& path) {
  ...
  if (v == ZOK) {
    ...
    if (pos == 0 || existsOrCreate(path.substr(0, pos))) {
      ...
      if (v != ZOK) {
        SYLAR_LOG_ERROR(g_logger) << "create path=" << path
                                  << " error:" << zerror(v) << " (" << v << ")";
        return false;
      }
      return true;
...
bool ParseDomainService(const std::string& path, std::string& domain,
                        std::string& service) {
...
bool ZKServiceDiscovery::queryInfo(const std::string& domain,
  ...
  if (service != "all") {
    ...
    std::string new_val(1024, 0);
    int32_t rt =
        m_client->create(path + "/" + m_selfInfo, m_selfData, new_val,
                         &ZOO_OPEN_ACL_UNSAFE, ZKClient::FlagsType::EPHEMERAL);
    if (rt == ZOK) {
      return true;
    }
