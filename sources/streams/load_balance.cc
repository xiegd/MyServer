...
void LoadBalanceItem::close() {
  if (m_stream) {
    ...
    sylar::WorkerMgr::GetInstance()->schedule("service_io",
                                              [stream]() { stream->close(); });
  }
}

bool LoadBalanceItem::isValid() { return m_stream && m_stream->isConnected(); }
...
HolderStatsSet::HolderStatsSet(uint32_t size) { m_stats.resize(size); }
...
void SDLoadBalance::stop() { m_sd->stop(); }
