...
RoaringBitmap::RoaringBitmap(uint32_t size) { m_bitmap.addRange(0, size); }

RoaringBitmap::RoaringBitmap(const RoaringBitmap& b) { m_bitmap = b.m_bitmap; }
...
bool RoaringBitmap::get(uint32_t idx) const { return m_bitmap.contains(idx); }
...
bool RoaringBitmap::any() const { return m_bitmap.begin() != m_bitmap.end(); }
...
uint32_t RoaringBitmap::getCount() const { return m_bitmap.cardinality(); }
