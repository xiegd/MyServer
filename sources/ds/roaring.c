...
int32_t intersect_skewed_uint16(const uint16_t *small, size_t size_s,
  ...
  while ((idx_s + 4 <= size_s) && (idx_l < size_l)) {
    ...
    binarySearch4(large + idx_l, (int32_t)(size_l - idx_l), target1, target2,
                  target3, target4, &index1, &index2, &index3, &index4);
  ...
  if ((idx_s + 2 <= size_s) && (idx_l < size_l)) {
    uint16_t target1 = small[idx_s];
    uint16_t target2 = small[idx_s + 1];
    binarySearch2(large + idx_l, (int32_t)(size_l - idx_l), target1, target2,
                  &index1, &index2);
    if ((index1 + idx_l < size_l) && (large[idx_l + index1] == target1)) {
      buffer[pos++] = target1;
    }
...
size_t bitset_extract_setbits_avx2(uint64_t *array, size_t length, void *vout,
  ...
  for (; (i < length) && (out < safeout); ++i) {
    ...
    while ((w != 0) && (out < safeout)) {
      uint64_t t = w & (~w + 1);   // on x64, should compile to BLSI (careful:
                                   // the Intel compiler seems to fail)
...
#ifdef USEAVX
    return intersect_vector16_cardinality(array1->array, card_1, array2->array,
                                          card_2);
#else
    return intersect_uint16_cardinality(array1->array, card_1, array2->array,
                                        card_2);
#endif
  }
}
