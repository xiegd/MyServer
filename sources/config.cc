  ...
  for (auto& i : all_nodes) {
    ...
    if (var) {
      ...
      if (i.second.IsScalar()) {
        var->fromString(
            i.second.Scalar());  // 获取YAML::Node对应的标量值的string形式，
      } else {
        std::stringstream ss;
...
static std::map<std::string, uint64_t>
    s_file2modifytime;  // 存储每个配置项的修改时间
static sylar::Mutex s_mutex;

// 遍历指定路径下的YAML配置文件，加载配置文件，更新配置信息, force是否强制加载
void Config::LoadFromConfDir(const std::string& path, bool force) {
  std::string absoulte_path = sylar::EnvMgr::GetInstance()->getAbsolutePath(
      path);  // GetInstance获取一个相应单例的类实例
  ...
  for (auto& i : files) {
    {  // 通过限制st的作用域，确保在代码结束时，st会自动销毁
      ...
      lstat(i.c_str(),
            &st);  // lstat系统调用将对应文件的状态信息填充到stat结构体中
