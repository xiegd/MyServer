...
static sylar::ConfigVar<std::map<
    std::string, std::map<std::string, std::string> > >::ptr g_thread_info_set =
...
FoxThreadPool::FoxThreadPool(uint32_t size, const std::string& name,
                             bool advance)
    : m_size(size),
      m_cur(0),
      m_name(name),
...
void FoxThreadManager::dispatch(const std::string& name, uint32_t id,
                                callback cb) {
  IFoxThread::ptr ti = get(name);
  SYLAR_ASSERT(ti);
  ti->dispatch(id, cb);
...
void FoxThreadManager::init() {
  ...
  for (auto i : m) {
    ...
    if (num <= 0) {
      SYLAR_LOG_ERROR(g_logger) << "thread pool:" << name << " num:" << num
    ...
    if (num == 1) {
      ...
      SYLAR_LOG_INFO(g_logger) << "init thread pool:" << name << " num:" << num
                               << " advance:" << advance;
    }
  }
}
