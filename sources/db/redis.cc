...
Redis::Redis() { m_type = IRedis::REDIS; }
...
bool Redis::connect(const std::string& ip, int port, uint64_t ms) {
  ...
  if (c) {
    ...
    if (!m_passwd.empty()) {
      ...
      if (!r) {
        SYLAR_LOG_ERROR(g_logger) << "auth error:(" << m_host << ":" << m_port
                                  << ", " << m_name << ")";
        return false;
      }
...
ReplyPtr Redis::cmd(const std::vector<std::string>& argv) {
  ...
  auto r =
      (redisReply*)redisCommandArgv(m_context.get(), argv.size(), &v[0], &l[0]);
  if (!r) {
    if (m_logEnable) {
...
RedisCluster::RedisCluster() { m_type = IRedis::REDIS_CLUSTER; }
...
bool RedisCluster::connect() { return connect(m_host, m_port, 50); }
...
void FoxRedis::ConnectCb(const redisAsyncContext* c, int status) {
  ...
  if (!status) {
    ...
    SYLAR_LOG_ERROR(g_logger) << "FoxRedis::ConnectCb " << c->c.tcp.host << ":"
...
ReplyPtr FoxRedis::cmd(const std::vector<std::string>& argv) {
  ...
  do {
    ...
    char* buf = nullptr;
    int len =
        redisFormatCommandArgv(&buf, argv.size(), &(args[0]), &(args_len[0]));
    if (len == -1 || !buf) {
...
void FoxRedisCluster::OnAuthCb(redisClusterAsyncContext* c, void* rp,
                               void* priv) {
...
void FoxRedisCluster::CmdCb(redisClusterAsyncContext* ac, void* r,
                            void* privdata) {
  Ctx* ctx = static_cast<Ctx*>(privdata);
  if (ctx->timeout) {
    delete ctx;
      ...
      if (type == "redis") {
        ...
        sylar::FoxThreadMgr::GetInstance()->dispatch(
            "redis", [this, conf, name, &done]() {
              sylar::FoxRedis* rds(
                  new sylar::FoxRedis(sylar::FoxThread::GetThis(), conf));
              rds->init();
              rds->setName(name);
