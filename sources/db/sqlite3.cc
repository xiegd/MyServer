...
int SQLite3::getErrno() { return sqlite3_errcode(m_db); }

std::string SQLite3::getErrStr() { return sqlite3_errmsg(m_db); }
...
int64_t SQLite3::getLastInsertId() { return sqlite3_last_insert_rowid(m_db); }
...
int64_t SQLite3Stmt::getLastInsertId() { return m_db->getLastInsertId(); }
...
int SQLite3Stmt::getErrno() { return m_db->getErrno(); }

std::string SQLite3Stmt::getErrStr() { return m_db->getErrStr(); }
...
int SQLite3Stmt::bind(int idx) { return sqlite3_bind_null(m_stmt, idx); }
...
int SQLite3Stmt::step() { return sqlite3_step(m_stmt); }

int SQLite3Stmt::reset() { return sqlite3_reset(m_stmt); }
...
int SQLite3Data::getDataCount() { return sqlite3_data_count(m_stmt->m_stmt); }
...
SQLite3Transaction::SQLite3Transaction(SQLite3::ptr db, bool auto_commit,
                                       Type type)
...
ISQLData::ptr SQLite3Manager::query(const std::string& name, const char* format,
  ...
  if (!conn) {
    SYLAR_LOG_ERROR(g_logger)
        << "SQLite3Manager::query, get(" << name << ") fail, format=" << format;
    return nullptr;
  }
