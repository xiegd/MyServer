...
static sylar::ConfigVar<std::unordered_map<
    std::string, std::unordered_map<std::string, std::string> > >::ptr
...
bool RockStream::RockSendCtx::doSend(AsyncSocketStream::ptr stream) {
  return std::dynamic_pointer_cast<RockStream>(stream)->m_decoder->serializeTo(
             stream, msg) > 0;
}
  ...
  if (type == Message::RESPONSE) {
    ...
    if (m_notifyHandler) {
      m_worker->schedule(std::bind(
          ...
          std::dynamic_pointer_cast<RockStream>(shared_from_this()), nty));
    } else {
...
RockConnection::RockConnection() : RockStream(nullptr) { m_autoConnect = true; }
