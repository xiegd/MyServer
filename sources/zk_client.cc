...
void ZKClient::OnWatcher(zhandle_t* zh, int type, int stat, const char* path,
                         void* watcherCtx) {
...
bool ZKClient::init(const std::string& hosts, int recv_timeout,
                    watcher_callback cb, log_callback lcb) {
  if (m_handle) {
    return true;
  }
  m_hosts = hosts;
  ...
  m_logCb = lcb;
  m_handle = zookeeper_init2(hosts.c_str(), &ZKClient::OnWatcher, m_recvTimeout,
                             nullptr, this, 0, lcb);
  return m_handle != nullptr;
}
...
int32_t ZKClient::create(const std::string& path, const std::string& val,
                         ...
                         int flags) {
  return zoo_create(m_handle, path.c_str(), val.c_str(), val.size(), acl, flags,
                    &new_path[0], new_path.size());
}
...
int32_t ZKClient::set(const std::string& path, const std::string& val,
...
int32_t ZKClient::getChildren(const std::string& path,
                              std::vector<std::string>& val, bool watch,
                              Stat* stat) {
...
int32_t ZKClient::getState() { return zoo_state(m_handle); }
