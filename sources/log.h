...
class LogLevel {
 public:
  ...
  static const char* ToString(
      LogLevel::Level level);  //将日志级别转化为文本输出
  static LogLevel::Level FromString(
      const std::string& str);  // 将文本转换为日志级别
};
...
class LogEvent {
 public:
  ...
  LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level,
           ...
           const std::string&
               thread_name);  // 没有初始化m_ss，根据m_logger来设置日志内容
...
// 日志事件包装器,
// 将日志事件和日志器包装到一起，方便通过宏定义来简化日志模块的使用
class LogEventWrap {
 public:
 ...
 public:
  ...
  class FormatItem {
   public:
    ...
    virtual ~FormatItem() {
    }  // 父类的destructor声明为virtual，保证释放派生类资源时，父类的析构函数被调用
    /**
     * @brief 格式化日志到流
...
class LogAppender {
 ...
 public:
  ...
  virtual std::string
  toYamlString() = 0;  // 将日志输出目标的配置转成YAML String
...
// 日志器，负责进行日志输出,
// 包含多个LogAppender和一个LogLevel，提供log方法，传入日志事件，判断日志级别然后输出
class Logger : public std::enable_shared_from_this<Logger> {
 ...
 public:
  ...
  void log(LogLevel::Level level,
           LogEvent::ptr event);  // 写日志, 需要指定LogLevel
  void debug(
      LogEvent::ptr event);  // 写debug级别日志, 在实现里面就是调用的前面的log，
  ...
  void setFormatter(
      const std::string& val);  // 设置日志格式模板, 实际调用上一个重载版本,
                                // 多了一个m_error的判断
  LogFormatter::ptr getFormatter();  // 获取日志格式器
...
class LoggerManager {
 public:
  ...
  Logger::ptr getLogger(
      const std::string& name);  // 获取日志器, name（日志器名称）,
                                 // 如果没有就新建一个相应bame的Logger
  void init();                                    // 初始化
