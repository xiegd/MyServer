...
static thread_local Fiber* t_fiber =
    nullptr;  // 当前线程正在运行的协程，必须时刻指向当前正在运行的协程对象
static thread_local Fiber::ptr t_threadFiber =
    nullptr;  // 当前线程的主协程，切换到主协程相当于切换到主线程中运行
// 查找协程栈大小对应的配置项
...
Fiber::Fiber(std::function<void()> cb, size_t stacksize, bool use_caller)
  ...
  m_stacksize =
      stacksize ? stacksize
                : g_fiber_stack_size->getValue();  // 获取相应配置项中设置的值
  ...
  if (!use_caller) {
    makecontext(&m_ctx, &Fiber::MainFunc,
                0);  // 将m_ctx和MainFunc绑定,
                     // 之后调用setcontext/swapcontext激活m_ctx后就会运行
  } else {
...
void Fiber::MainFunc() {
  ...
  raw_ptr->swapOut();  // Fiber类的指针调用swapout()将当前协程切换到后台,
                       // 为什么使用裸指针调用??
