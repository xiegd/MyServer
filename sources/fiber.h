...
class Fiber : public std::enable_shared_from_this<Fiber> {
 ...
 private
     :  // private，只有通过GetThis()返回当前正在运行的协程时，如果发现主协程未被初始化则调用
  Fiber();  // 无参数构造函数，初始化当前线程的协程功能，构造线程主协程对象

 public:
  // 构造函数，用于构造子协程,
  // cb，协程执行的函数(入口函数)；stacksize，协程栈大小；use_caller，是否在MainFiber上调度
  ...
  void
  back();  // 将当前线程切换到后台，pre执行的为该协程，post返回到线程的主协程
