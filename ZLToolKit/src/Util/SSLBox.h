...
class SSL_Initor {
   public:
     ...
     * The certificate file must contain both public and private keys (cer
     format certificates only include public keys, use the following method to
     load)
     * The client can load the certificate by default (unless the server
     ...
     * @return 是否加载成功
     * Trust a certain certificate, generally used for clients to trust
     self-signed certificates or certificates signed by self-signed CAs
     * For example, if my client wants to trust a certificate I issued myself,
     we can only trust this certificate
     ...
     */
    bool trustCertificate(const std::string &pem_p12_cer,
     ...
     */
    std::shared_ptr<SSL_CTX> getSSLCtx(const std::string &vhost,
                                       bool server_mode);
   ...
   private:
     ...
     */
    bool setContext(const std::string &vhost,
                    const std::shared_ptr<SSL_CTX> &ctx, bool server_mode,
                    bool is_default = true);
    ...
    std::shared_ptr<SSL_CTX> getSSLCtx_l(const std::string &vhost,
                                         bool server_mode);

    std::shared_ptr<SSL_CTX> getSSLCtxWildcards(const std::string &vhost,
                                                bool server_mode);
   ...
   private:
    ...
    std::map<std::string, std::shared_ptr<SSL_CTX>, less_nocase>
        _ctxs_wildcards[2];
};
...
class SSL_Box {
   public:
    SSL_Box(bool server_mode = true, bool enable = true,
