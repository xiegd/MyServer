...
SSL_Initor::SSL_Initor() {
    ...
    setContext(
        "",
        SSLUtil::makeSSLContext(vector<shared_ptr<X509> >(), nullptr, false),
        false);
    setContext(
        "", SSLUtil::makeSSLContext(vector<shared_ptr<X509> >(), nullptr, true),
        true);
#endif  // defined(ENABLE_OPENSSL)
}
...
bool SSL_Initor::loadCertificate(const string &pem_or_p12, bool server_mode,
                                 const string &password, bool is_file,
                                 bool is_default) {
...
bool SSL_Initor::setContext(const string &vhost, const shared_ptr<SSL_CTX> &ctx,
...
void SSL_Initor::setupCtx(SSL_CTX *ctx) {
    ...
    SSL_CTX_set_verify(
        ctx, SSL_VERIFY_NONE, [](int ok, X509_STORE_CTX *pStore) {
            if (!ok) {
...
bool SSL_Initor::trustCertificate(const string &pem_p12_cer, bool server_mode,
                                  const string &password, bool is_file) {
...
std::shared_ptr<SSL_CTX> SSL_Initor::getSSLCtx(const string &vhost,
                                               bool server_mode) {
...
std::shared_ptr<SSL_CTX> SSL_Initor::getSSLCtxWildcards(const string &vhost,
                                                        bool server_mode) {
...
std::shared_ptr<SSL_CTX> SSL_Initor::getSSLCtx_l(const string &vhost_in,
                                                 bool server_mode) {
    auto vhost = vhost_in;
    if (vhost.empty()) {
...
SSL_Box::SSL_Box(bool server_mode, bool enable, int buff_size) {
    ...
    if (_ssl) {
        ...
        _server_mode ? SSL_set_accept_state(_ssl.get())
...
void SSL_Box::flushReadBio() {
    ...
    do {
        nread =
            SSL_read(_ssl.get(), buffer_bio->data() + total, buf_size - total);
        if (nread > 0) {
            total += nread;
        }
    } while (nread > 0 && buf_size - total > 0);
...
void SSL_Box::flush() {
    ...
    onceToken token([&] { _is_flush = true; }, [&]() { _is_flush = false; });
