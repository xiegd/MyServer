...
class Option {
   public:
    using OptionHandler = std::function<bool(
        const std::shared_ptr<std::ostream> &stream, const std::string &arg)>;
    ...
    Option(char short_opt, const char *long_opt, enum ArgType type,
    ...
    bool operator()(const std::shared_ptr<std::ostream> &stream,
...
class OptionParser {
   public:
    using OptionCompleted =
        std::function<void(const std::shared_ptr<std::ostream> &, mINI &)>;

    OptionParser(const OptionCompleted &cb = nullptr,
                 bool enable_empty_args = true) {
        _on_completed = cb;
        _enable_empty_args = enable_empty_args;
        _helper = Option(
            'h', "help", Option::ArgNone, nullptr, false, "打印此信息",
            [this](const std::shared_ptr<std::ostream> &stream,
                ...
                for (auto &pr : _map_options) {
                    ...
                    for (size_t i = 0;
                         i < maxLen_longOpt - opt._long_opt.size(); ++i) {
                        printer << " ";
                    }
                    //打印是否有参
                    ...
                    for (size_t i = 0; i < maxLen_default - defaultValue.size();
                         ++i) {
                        printer << " ";
                    }
                    //打印是否必填参数
    ...
    void operator()(mINI &all_args, int argc, char *argv[],
...
class CMD : public mINI {
   public:
    ...
    virtual const char *description() const { return "description"; }
    ...
    bool hasKey(const char *key) { return this->find(key) != this->end(); }
...
class CMDRegister {
   public:
    ...
    std::shared_ptr<CMD> operator[](const char *name) {
        ...
        if (it == _cmd_map.end()) {
            throw std::invalid_argument(std::string("CMD not existed: ") +
                                        name);
        }
        return it->second;
    }
...
class CMD_help : public CMD {
   public:
    ...
    const char *description() const override { return "打印帮助信息"; }
};
...
class CMD_exit : public CMD {
   public:
    CMD_exit() {
        _parser = std::make_shared<OptionParser>(
            [](const std::shared_ptr<std::ostream> &, mINI &) {
                throw ExitException();
            });
    }

    const char *description() const override { return "退出shell"; }
};
...
class CMD_clear : public CMD {
   public:
    ...
    const char *description() const { return "清空屏幕输出"; }
...
#define REGIST_CMD(name) \
    CMDRegister::Instance().registCMD(#name, std::make_shared<CMD_##name>());
