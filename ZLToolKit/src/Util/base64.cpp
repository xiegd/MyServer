...
int av_base64_decode(uint8_t *out, const char *in, int out_size) {
    ...
    for (i = 0; in[i] && in[i] != '='; i++) {
        ...
        if (index >= FF_ARRAY_ELEMS(map2) || map2[index] == 0xff) return -1;
        v = (v << 6) + map2[index];
        if (i & 3) {
            if (dst - out < out_size) {
                *dst++ = v >> (6 - 2 * (i & 3));
...
char *av_base64_encode_l(char *out, int *out_size, const uint8_t *in,
                         int in_size) {
...
char *av_base64_encode(char *out, int out_size, const uint8_t *in,
                       int in_size) {
...
string encodeBase64(const string &txt) {
    ...
    if (!av_base64_encode_l((char *)ret.data(), &size,
