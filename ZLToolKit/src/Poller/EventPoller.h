...
class EventPoller : public TaskExecutor,
   ...
   public:
     ...
     * @param may_sync If the calling thread is the polling thread of this
     object,
     *                  then if may_sync is true, the task will be executed
     synchronously
     ...
     * @return 是否为本对象的轮询线程
     * Checks if the thread calling this interface is the polling thread of this
     object
     ...
     *              If an exception is thrown in the task, it defaults to not
     repeating the task.
   ...
   private:
     ...
     * @param blocked Whether to execute polling with the thread that calls this
     interface
     ...
     * @param may_sync
     * @param first
     * @return The cancellable task itself, or nullptr if it has been executed
     synchronously
...
#define EventPollerPoolOnStartedArgs EventPollerPool &pool, size_t &size
     ...
     * Whether to set CPU affinity for internal thread creation, default is to
     set CPU affinity
     ...
     * @param prefer_current_thread Whether to prioritize getting the current
     thread
     ...
     * When creating Socket objects in batches, if prioritizing the current
     thread,
     * it will cause the load to be unbalanced, so it can be temporarily closed
     and then reopened
