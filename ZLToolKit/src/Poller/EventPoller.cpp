...
int EventPoller::addEvent(int fd, int event, PollEventCB cb) {
    ...
    if (isCurrentThread()) {
        ...
        // [AUTO-TRANSLATED:6adfc664]
        // On the win32 platform, the socket does not equal the file descriptor,
    ...
    if (!flush) {
        for (;;) {
            ...
            if (err == 0 || get_uv_error(true) != UV_EAGAIN) {
                ...
                // [AUTO-TRANSLATED:5f7a013d]
                // Received eof or non-EAGAIN (no more data) error, indicating
                // that the pipe is invalid, reopen the pipe
...
SocketRecvBuffer::Ptr EventPoller::getSharedBuffer(bool is_udp) {
    ...
    // [AUTO-TRANSLATED:2d2ee7bf]
    // On non-Linux platforms, tcp and udp share the recvfrom scheme, using the
    // same buffer
    is_udp = 0;
#endif
