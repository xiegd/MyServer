...
class SockUtil {
   public:
     ...
     */
    static int listen(const uint16_t port,
                      const char *local_ip = "::", int back_log = 1024);
     ...
     * @param fd socket fd号
     * @return 0代表成功，-1为失败
     * Write socket does not trigger SIG_PIPE signal (seems to be effective only
     on Mac)
     * @param fd socket fd number
     * @return 0 represents success, -1 represents failure
     ...
     * @param size 接收缓存大小
     * @return 0代表成功，-1为失败
     * Set the socket receive buffer, default is around 8K, generally has an
     upper limit
     ...
     */
    static int joinMultiAddr(int fd, const char *addr,
     ...
     */
    static int leaveMultiAddr(int fd, const char *addr,
                              const char *local_ip = "0.0.0.0");
     ...
     */
    static int joinMultiAddrFilter(int sock, const char *addr,
                                   const char *src_ip,
                                   const char *local_ip = "0.0.0.0");
     ...
     */
    static int leaveMultiAddrFilter(int fd, const char *addr,
                                    const char *src_ip,
                                    const char *local_ip = "0.0.0.0");
