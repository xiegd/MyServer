...
class Session : public SocketHelper {
   public:
     ...
     * @param server, 服务器对象
     * After creating a Session, the Server will pass its configuration
     parameters to the Session through this function
   ...
   private:
    ...
    std::unique_ptr<toolkit::ObjectStatistic<toolkit::TcpSession> >
        _statistic_tcp;
    std::unique_ptr<toolkit::ObjectStatistic<toolkit::UdpSession> >
        _statistic_udp;
};
...
class SessionWithSSL : public SessionType {
   public:
    ...
    SessionWithSSL(ArgsType &&...args)
        ...
        _ssl_box.setOnEncData(
            [&](const Buffer::Ptr &buf) { public_send(buf); });
        _ssl_box.setOnDecData(
            [&](const Buffer::Ptr &buf) { public_onRecv(buf); });
    }
    ...
    // [AUTO-TRANSLATED:7b16e05b]
    // Adding public_onRecv and public_send functions is to solve a bug in lower
    // versions of gcc where a lambda cannot access protected or private methods
