...
class SessionMap : public std::enable_shared_from_this<SessionMap> {
   public:
    ...
    static SessionMap &Instance();  //单例  
    ~SessionMap() = default;

    //获取Session  
    Session::Ptr get(const std::string &tag);
    void for_each_session(
        const std::function<void(const std::string &id,
                                 const Session::Ptr &session)> &cb);

   private:
    SessionMap() = default;

    bool del(const std::string &tag);  //移除Session  
    bool add(const std::string &tag, const Session::Ptr &session); //添加Session  
...
class SessionHelper {
   ...
   private:
    ...
    Session::Ptr _session;  // 实际管理的会话
    SessionMap::Ptr _session_map;  // 服务器管理的全局会话map
    std::weak_ptr<Server> _server;  // 服务器对象的弱引用
};
