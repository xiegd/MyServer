...
class TcpClientWithSSL : public TcpClientType {
   public:
    ...
    void startConnectWithProxy(const std::string &url,
                               const std::string &proxy_host,
                               uint16_t proxy_port, float timeout_sec = 5,
                               uint16_t local_port = 0) override {
        _host = url;
        TcpClientType::startConnect(proxy_host, proxy_port, timeout_sec,
                                    local_port);
    }
    ...
    void onConnect(const SockException &ex) override {
        if (!ex) {
            ...
            _ssl_box->setOnDecData(
                [this](const Buffer::Ptr &buf) { public_onRecv(buf); });
