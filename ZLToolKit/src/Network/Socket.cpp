...
StatisticImp(Socket)
    ...
    switch (error) {
        ...
        case UV_ECONNRESET:
            return SockException(Err_reset, uv_strerror(error), error);
        default:
...
void Socket::setOnRead(onReadCB cb) {
    onMultiReadCB cb2;
    if (cb) {
        cb2 = [cb](Buffer::Ptr *buf, struct sockaddr_storage *addr,
                   size_t count) {
            for (auto i = 0u; i < count; ++i) {
                cb(buf[i], (struct sockaddr *)(addr + i),
...
void Socket::setOnMultiRead(onMultiReadCB cb) {
    ...
    if (cb) {
        ...
        _on_multi_read = [](Buffer::Ptr *buf, struct sockaddr_storage *addr,
                            size_t count) {
            for (auto i = 0u; i < count; ++i) {
                WarnL << "Socket not set read callback, data ignored: "
                      << buf[i]->size();
            }
        };
    }
...
void Socket::setOnAccept(onAcceptCB cb) {
    ...
    if (cb) {
        ...
        _on_accept = [](Socket::Ptr &sock, shared_ptr<void> &complete) {
            WarnL << "Socket not set accept callback, peer fd: "
                  << sock->rawFD();
        };
    }
}
...
void Socket::connect(const string &url, uint16_t port, const onErrCB &con_cb_in,
                     float timeout_sec, const string &local_ip,
                     uint16_t local_port) {
    weak_ptr<Socket> weak_self = shared_from_this();
    // 因为涉及到异步回调，所以在poller线程中执行确保线程安全
    // [AUTO-TRANSLATED:e4b29f5e]
    // Because it involves asynchronous callbacks, execute in the poller thread
...
void Socket::connect_l(const string &url, uint16_t port,
    ...
    auto async_con_cb = std::make_shared<function<void(const SockNum::Ptr &)>>(
        [weak_self, con_cb](const SockNum::Ptr &sock) {
            ...
            // [AUTO-TRANSLATED:e9809ee3]
            // Listen for whether the socket is writable, writable indicates
            // that the connection to the server is successful
            ...
            if (result == -1) {
                con_cb(SockException(
                    Err_other,
                    std::string(
                        "add event to poller failed when start connect:") +
                        get_uv_errmsg()));
            } else {
                // 先创建SockFD对象，防止SockNum由于未执行delEvent无法析构
                // [AUTO-TRANSLATED:99d4e610]
                // First create the SockFD object to prevent SockNum from being
                // destructed due to not executing delEvent
...
void Socket::onConnected(const SockNum::Ptr &sock, const onErrCB &cb) {
    ...
    if (!attachEvent(sock)) {
        ...
        cb(SockException(Err_other,
                         "add event to poller failed when connected"));
        return;
    }
...
bool Socket::attachEvent(const SockNum::Ptr &sock) {
    ...
    if (sock->type() == SockNum::Sock_TCP_Server) {
        ...
        // TCP server
        auto result = _poller->addEvent(
            sock->rawFd(), EventPoller::Event_Read | EventPoller::Event_Error,
    ...
    // TCP client or UDP
    auto read_buffer =
        _poller->getSharedBuffer(sock->type() == SockNum::Sock_UDP);
    auto result = _poller->addEvent(
        sock->rawFd(),
        EventPoller::Event_Read | EventPoller::Event_Error |
            EventPoller::Event_Write,
...
ssize_t Socket::onRead(const SockNum::Ptr &sock,
    ...
    while (_enable_recv) {
        ...
        if (nread == -1) {
            ...
            if (err != UV_EAGAIN) {
                if (sock->type() == SockNum::Sock_TCP) {
                    ...
                    WarnL << "Recv err on udp socket[" << sock->rawFd()
                          << "]: " << uv_strerror(err);
                }
            }
            return ret;
        ...
        try {
            ...
            // [AUTO-TRANSLATED:2f3f813b]
            // Catch exception here, the purpose is to prevent data from not
            // being read completely, and the epoll edge trigger fails
            LOCK_GUARD(_mtx_event);
...
bool Socket::emitErr(const SockException &err) noexcept {
    ...
    _poller->async([weak_self, err]() {
        ...
        // [AUTO-TRANSLATED:db5a0958]
        // Delay closing the socket, only remove its IO event, to prevent
        // Session object destruction from failing to get fd related information
...
ssize_t Socket::send(string buf, struct sockaddr *addr, socklen_t addr_len,
                     bool try_flush) {
    return send(std::make_shared<BufferString>(std::move(buf)), addr, addr_len,
                try_flush);
}

ssize_t Socket::send(Buffer::Ptr buf, struct sockaddr *addr, socklen_t addr_len,
                     bool try_flush) {
    if (!addr) {
        if (!_udp_send_dst) {
        ...
        // [AUTO-TRANSLATED:afb6ce35]
        // This send did not specify a target address, but the target is
        // customized through bindPeerAddr
...
int Socket::flushAll() {
    ...
    if (_send_flush_ticker.elapsedTime() > _max_send_buffer_ms) {
        ...
        // [AUTO-TRANSLATED:19ee680e]
        // If the oldest data in the send queue exceeds the timeout limit,
...
size_t Socket::getSendBufferCount() {
    ...
    {
        LOCK_GUARD(_mtx_send_buf_sending);
        _send_buf_sending.for_each(
            [&](BufferList::Ptr &buf) { ret += buf->count(); });
    }
    return ret;
}
    ...
    while (true) {
        if (event & EventPoller::Event_Read) {
            ...
            if (fd == -1) {
                ...
                // emitErr(ex);
                // https://github.com/ZLMediaKit/ZLMediaKit/issues/2946
                ErrorL << "Accept socket failed: " << ex.what();
                // 可能打开的文件描述符太多了:UV_EMFILE/UV_ENFILE
                // [AUTO-TRANSLATED:ecd1b4f1]
                ...
                _poller->doDelayTask(100, [weak_self, sock]() {
                    if (auto strong_self = weak_self.lock()) {
                        ...
                        // [AUTO-TRANSLATED:532951a2]
                        // Process the accept event again after 100ms, maybe
                        // there are available fds
                        strong_self->onAccept(sock, EventPoller::Event_Read);
                    }
                    return 0;
                });
                // 暂时不处理accept事件，等待100ms后手动触发onAccept(只有EAGAIN读空后才能通过epoll再次触发事件)
                // [AUTO-TRANSLATED:32636aea]
                // Temporarily do not process the accept event, wait 100ms and
                // manually trigger onAccept (can only be triggered again
                // through epoll after EAGAIN reads empty)
                return -1;
#else
                // 水平触发；休眠10ms，防止无谓的accept失败
                // [AUTO-TRANSLATED:6f8349bb]
                // Level trigger; sleep 10ms to prevent unnecessary accept
                // failures
                this_thread::sleep_for(std::chrono::milliseconds(10));
                // 暂时不处理accept事件，由于是水平触发，下次还会再次自动进入onAccept函数
                // [AUTO-TRANSLATED:9aec1432]
                // Temporarily do not process the accept event, as it is level
                // trigger, it will automatically enter the onAccept function
                // again next time
                return -1;
#endif
            }
            ...
            try {
                ...
                // [AUTO-TRANSLATED:523d496d]
                // Catch exceptions here to prevent the problem of epoll edge
                // trigger failure when the socket is not fully accepted
                LOCK_GUARD(_mtx_event);
                // 拦截Socket对象的构造  [AUTO-TRANSLATED:b38b67b9]
                // Intercept the Socket object's constructor
                peer_sock = _on_before_accept(_poller);
            } catch (std::exception &ex) {
                ErrorL << "Exception occurred when emit on_before_accept: "
                       << ex.what();
                close(fd);
                continue;
            }
            ...
            if (!peer_sock) {
                ...
                // [AUTO-TRANSLATED:6c057de0]
                // This is the default construction behavior, which means the
                // child Socket shares the parent Socket's poll thread and
                // closes the mutex lock
            ...
            // [AUTO-TRANSLATED:e3e3c225]
            // Set the fd properly, so that it can be accessed normally in the
            // onAccept event
            peer_sock->setSock(sock);
            // 赋值peer ip，防止在执行setSock时，fd已经被reset断开
            // [AUTO-TRANSLATED:7ca197db]
            // Assign the peer ip to prevent the fd from being reset and
            // disconnected when executing setSock
            ...
            shared_ptr<void> completed(nullptr, [peer_sock, sock](void *) {
                try {
                    ...
                    // [AUTO-TRANSLATED:45618926]
                    // Then add the fd to the poll monitoring (ensure that the
                    // onAccept event is triggered first, followed by onRead and
                    // other events)
                    if (!peer_sock->attachEvent(sock)) {
                        // 加入poll监听失败，触发onErr事件，通知该Socket无效
                        // [AUTO-TRANSLATED:e81fd478]
                        // If adding to poll monitoring fails, trigger the onErr
                        // event to notify that the Socket is invalid
            ...
            try {
                // 此处捕获异常，目的是防止socket未accept尽，epoll边沿触发失效的问题
                // [AUTO-TRANSLATED:523d496d]
                // Catch exceptions here to prevent the problem of socket not
                // being accepted and epoll edge triggering failure
                LOCK_GUARD(_mtx_event);
                // 先触发onAccept事件，此时应该监听该Socket的onRead等事件
                // [AUTO-TRANSLATED:29734871]
                // First trigger the onAccept event, at this point, you should
                // listen for onRead and other events of the Socket
                _on_accept(peer_sock, completed);
            } catch (std::exception &ex) {
                ErrorL << "Exception occurred when emit on_accept: "
                       << ex.what();
                continue;
            }
        }
...
bool Socket::flushData(const SockNum::Ptr &sock, bool poller_thread) {
        ...
        do {
            {
                ...
                // [AUTO-TRANSLATED:8ddb2962]
                // The secondary send cache is empty, so we continue to consume
                // data from the primary cache
                LOCK_GUARD(_mtx_send_buf_waiting);
                if (!_send_buf_waiting.empty()) {
                    // 把一级缓中数数据放置到二级缓存中并清空
                    // [AUTO-TRANSLATED:4884aa58]
                    // Put the data from the first-level cache into the
                    // second-level cache and clear it
                    LOCK_GUARD(_mtx_event);
                    ...
                    send_buf_sending_tmp.emplace_back(BufferList::create(
                        std::move(_send_buf_waiting), std::move(send_result),
                        sock->type() == SockNum::Sock_UDP));
                    break;
                }
            }
            // 如果一级缓存也为空,那么说明所有数据均写入socket了
            // [AUTO-TRANSLATED:6ae9ef8a]
            // If the first-level cache is also empty, it means that all data
            // has been written to the socket
            if (poller_thread) {
                // poller线程触发该函数，那么该socket应该已经加入了可写事件的监听；
                // [AUTO-TRANSLATED:5a8e123d]
                // The poller thread triggers this function, so the socket
                // should have been added to the writable event listening
                // 那么在数据列队清空的情况下，我们需要关闭监听以免触发无意义的事件回调
                // [AUTO-TRANSLATED:0fb35573]
                // So, in the case of data queue clearing, we need to close the
                // listening to avoid triggering meaningless event callbacks
    ...
    while (!send_buf_sending_tmp.empty()) {
        ...
        if (n > 0) {
            ...
            if (!poller_thread) {
                ...
                // [AUTO-TRANSLATED:917049f0]
                // If this function is triggered by the poller thread, the
                // socket should have been added to the writable event
                // listening, so we don't need to add listening again
        ...
        if (err == UV_EAGAIN) {
            ...
            if (!poller_thread) {
                ...
                // [AUTO-TRANSLATED:917049f0]
                // If this function is triggered by the poller thread, the
                // socket should have already been added to the writable event
                // listener, so we don't need to add it again
                startWriteAbleEvent(sock);
            }
            break;
        }
        ...
        if (sock->type() == SockNum::Sock_UDP) {
            ...
            send_buf_sending_tmp.pop_front();
            WarnL << "Send udp socket[" << sock
                  << "] failed, data ignored: " << uv_strerror(err);
            continue;
        }
    ...
    if (!send_buf_sending_tmp.empty()) {
        ...
        // [AUTO-TRANSLATED:2d7f9f2f]
        // The secondary cache has not been sent completely, indicating that the
        // socket is not writable, return directly
        return true;
    }

    // 二级缓存已经全部发送完毕，说明该socket还可写，我们尝试继续写
    // [AUTO-TRANSLATED:2c2bc316]
    // The secondary cache has been sent completely, indicating that the socket
    // is still writable, we try to continue writing
    // 如果是poller线程，我们尝试再次写一次(因为可能其他线程调用了send函数又有新数据了)
    // [AUTO-TRANSLATED:392684a8]
    // If it's the poller thread, we try to write again (because other threads
    // may have called the send function and there is new data)
...
void Socket::startWriteAbleEvent(const SockNum::Ptr &sock) {
    ...
    _poller->modifyEvent(
        sock->rawFd(),
        flag | EventPoller::Event_Error | EventPoller::Event_Write,
        [sock](bool) {});
}
...
bool Socket::bindPeerAddr(const struct sockaddr *dst_addr, socklen_t addr_len,
    ...
    if (soft_bind) {
        ...
        // [AUTO-TRANSLATED:f3f26702]
        // After hard binding, cancel soft binding to prevent performance loss
        // of memcpy target address
        _udp_send_dst = nullptr;
        if (-1 == ::connect(_sock_fd->rawFd(), dst_addr, addr_len)) {
            WarnL << "Connect socket to peer address failed: "
                  << SockUtil::inet_ntoa(dst_addr);
            return false;
        }
...
void Socket::setSendFlags(int flags) { _sock_flags = flags; }
...
const Socket::Ptr &SocketHelper::getSock() const { return _sock; }
...
string SocketHelper::get_peer_ip() { return _sock ? _sock->get_peer_ip() : ""; }
...
void SocketHelper::setSendFlushFlag(bool try_flush) { _try_flush = try_flush; }
