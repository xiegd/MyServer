...
Socket::Ptr Socket::createSocket(const EventPoller::Ptr &poller_in,
    ...
    // 使用shared_ptr管理Socket对象，指定deleter
...
void Socket::connect(const string &url, uint16_t port, const onErrCB &con_cb_in,
    ...
    _poller->async([=] {
        // 检查socket是否还存在（weak_ptr不占用引用计数）
        // lock()方法如果存在返回一个新的shared_ptr，否则返回nullptr
