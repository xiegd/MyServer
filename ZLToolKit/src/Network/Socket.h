...
//[AUTO-TRANSLATED:9c5d8d87] Send timeout time, if no data is sent successfully
// within the specified time, the onErr event will be triggered
...
class SockFD : public noncopyable {
   public:
    ...
    void setConnected() { _num->setConnected(); }

    int rawFd() const { return _num->rawFd(); }

    const SockNum::Ptr &sockNum() const { return _num; }

    SockNum::SockType type() { return _num->type(); }
...
#define TraceP(ptr)                                                    \
    TraceL << ptr->getIdentifier() << "(" << ptr->get_peer_ip() << ":" \
           << ptr->get_peer_port() << ") "
#define DebugP(ptr)                                                    \
    DebugL << ptr->getIdentifier() << "(" << ptr->get_peer_ip() << ":" \
           << ptr->get_peer_port() << ") "
#define InfoP(ptr)                                                    \
    InfoL << ptr->getIdentifier() << "(" << ptr->get_peer_ip() << ":" \
          << ptr->get_peer_port() << ") "
#define WarnP(ptr)                                                    \
    WarnL << ptr->getIdentifier() << "(" << ptr->get_peer_ip() << ":" \
          << ptr->get_peer_port() << ") "
#define ErrorP(ptr)                                                    \
    ErrorL << ptr->getIdentifier() << "(" << ptr->get_peer_ip() << ":" \
           << ptr->get_peer_port() << ") "
...
class Socket : public std::enable_shared_from_this<Socket>,
   ...
   public:
    ...
    // [AUTO-TRANSLATED:2dd1c036] Socket send buffer is cleared event, returns
    // true to continue listening for the event next time, otherwise stops
    using onFlush = std::function<bool()>;
    //在接收到连接请求前，拦截Socket默认生成方式  [AUTO-TRANSLATED:2f07f268]
    // Intercept the default generation method of the Socket before receiving a
    // connection request
     ...
     * @param enable_mutex Whether to enable the mutex (whether the interface is
     thread-safe)

     * [AUTO-TRANSLATED:39bd767a]
    */
    static Ptr createSocket(const EventPoller::Ptr &poller = nullptr,
     ...
     */
    void connect(const std::string &url, uint16_t port, const onErrCB &con_cb,
                 float timeout_sec = 5,
    ...
    bool listen(uint16_t port,
                const std::string &local_ip = "::", int backlog = 1024);
     ...
     * @return 是否成功
     * Create a UDP socket, UDP is connectionless, so it can be used as a server
     and client
     ...
     */
    bool bindUdpSock(uint16_t port, const std::string &local_ip = "::",
     ...
     * Wrap an external file descriptor, this object is responsible for closing
     the file descriptor
     * Internally, the file descriptor will be set to NoBlocked, NoSigpipe,
     CloExec
     ...
     * The purpose is to allow a socket to be listened to by multiple poller
     objects, improving performance or implementing socket migration between
     threads
     ...
     * @return -1 represents failure (invalid socket), 0 represents data length
     is 0, otherwise returns data length
     ...
     * @return -1 represents failure (invalid socket or send timeout), 0
     represents success?
     ...
     * Close the socket and trigger the onErr callback, the onErr callback will
     be executed in the poller thread
     ...
     * @return 文件描述符
     * Get the raw file descriptor, do not perform close operation (because the
     Socket object will manage its lifecycle)
     ...
     * @return 套接字是否忙
     * Whether the socket is busy, if the socket write buffer is full, returns
     true
     ...
     * @param dst_addr 目标地址
     * @param addr_len 目标地址长度
     * @param soft_bind 是否软绑定，软绑定时不调用udp
     connect接口，只保存目标地址信息，发送时再传递到sendto函数
     * @return 是否成功
     * Binds the UDP target address, subsequent sends do not need to specify it
     separately
     * @param dst_addr Target address
     * @param addr_len Target address length
     * @param soft_bind Whether to soft bind, soft binding does not call the UDP
     connect interface, only saves the target address information, and sends it
     ...
     * Closes the socket
     * @param close_fd Whether to close the fd or only remove the IO event
     listener
     ...
     * Gets the number of milliseconds since the last socket send buffer was
   ...
   private:
    ...
    // [AUTO-TRANSLATED:3bd6dba3]
    // Maximum send buffer, in milliseconds, the time since the last send buffer
    // was cleared cannot exceed this parameter
    uint32_t _max_send_buffer_ms = SEND_TIME_OUT_SEC * 1000;
    // 控制是否接收监听socket可读事件，关闭后可用于流量控制
    // [AUTO-TRANSLATED:71de6ece]
    // Control whether to receive listen socket readable events, can be used for
    // traffic control after closing
    std::atomic<bool> _enable_recv{true};
    // 标记该socket是否可写，socket写缓存满了就不可写 [AUTO-TRANSLATED:32392de2]
    // Mark whether the socket is writable, the socket write buffer is full and
    // cannot be written
    ...
    // [AUTO-TRANSLATED:2c44d156]
    // Record the timer for the last send buffer (including socket write buffer
    // and application layer buffer) cleared
    ...
    // The poller thread bound to this socket, events are triggered in this
    // thread
    EventPoller::Ptr _poller;
    ...
    // [AUTO-TRANSLATED:da85b845]
    // TCP listener receives an accept request, custom creation of peer Socket
    // event (can control binding of child Socket to other poller threads)
    ...
    // [AUTO-TRANSLATED:26f1da58]
    // First-level send cache, when the socket is writable, it will batch the
    // first-level cache into the second-level cache
    ...
    // [AUTO-TRANSLATED:cc665665]
    // Second-level send cache, when the socket is writable, it will batch the
    // second-level cache into the socket
    ...
    // [AUTO-TRANSLATED:f8847463]
    // Connection cache address, to prevent TCP reset from causing the inability
    // to obtain the peer's address
...
class SockSender {
   public:
    ...
    virtual void shutdown(const SockException &ex =
                              SockException(Err_shutdown, "self shutdown")) = 0;
...
class SocketHelper : public SockSender,
   ...
   public:
    ...
    /**
     * 套接字是否忙，如果套接字写缓存已满则返回true
     * Whether the socket is busy, returns true if the socket write buffer is
     full
     ...
     * Safely detach from the Server and trigger the onError event in a
     thread-safe manner
     * @param ex The reason for triggering the onError event

     * [AUTO-TRANSLATED:739455d5]
     */
    void safeShutdown(const SockException &ex = SockException(Err_shutdown,
                                                              "self shutdown"));
     ...
     * 收到该事件时, 该对象一般将立即被销毁
     * @param err 原因
     * Callback received eof or other events that cause disconnection from
     Server
     * When this event is received, the object is generally destroyed
     immediately
     * @param err reason
