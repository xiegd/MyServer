...
// 生成ipv4/v6标识符时，为统一格式给ipv4的填充, 标准的ipv4映射前缀
// 规定的填充标准格式，区分普通的ipv6和ipv4映射地址
...
// 生成固定18bytes长度的peer id标识符，2Bytes的端口号 + 16Bytes的IP地址
static UdpServer::PeerIdType makeSockId(sockaddr *addr, int) {
    ...
    switch (addr->sa_family) {
        case AF_INET: {
            ret.resize(18);
            // 提取端口号的高8位和低8位
            ret[0] = ((struct sockaddr_in *)addr)->sin_port >> 8;  // 右移8位，获取高8位
            ret[1] = ((struct sockaddr_in *)addr)->sin_port & 0xFF;  // 与0xFF进行按位与操作，获取低8位
            // ipv4地址统一转换为ipv6方式处理
