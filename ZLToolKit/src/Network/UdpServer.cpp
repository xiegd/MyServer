...
void UdpServer::setupEvent() {
    ...
    _socket->setOnRead(
        [weak_self](Buffer::Ptr &buf, struct sockaddr *addr, int addr_len) {
...
UdpServer::~UdpServer() {
    if (!_cloned && _socket && _socket->rawFD() != -1) {
        InfoL << "Close udp server [" << _socket->get_local_ip()
...
void UdpServer::start_l(uint16_t port, const std::string &host) {
    ...
    //[AUTO-TRANSLATED:113cf4fd] Only the main server creates a session map,
    // other cloned servers share it
    _session_mutex = std::make_shared<std::recursive_mutex>();
    _session_map =
        std::make_shared<std::unordered_map<PeerIdType, SessionHelper::Ptr> >();

    // 新建一个定时器定时管理这些 udp
    // 会话,这些对象只由主server做超时管理，cloned server不管理
    // [AUTO-TRANSLATED:d20478a2]
    // Create a timer to manage these udp sessions periodically, these objects
    // are only managed by the main server, cloned servers do not manage them
    std::weak_ptr<UdpServer> weak_self =
        std::static_pointer_cast<UdpServer>(shared_from_this());
    ...
    if (_multi_poller) {
        ...
        // [AUTO-TRANSLATED:15a85c8f]
        // Clone the server to different threads to support multi-threading for
        // the udp server
    ...
    if (!_socket->bindUdpSock(port, host.c_str())) {
        ...
        // [AUTO-TRANSLATED:c31eedba]
        // Failed to bind udp port, possibly due to port occupation or
        // permission issues
        std::string err =
            (StrPrinter << "Bind udp socket on " << host << " " << port
    ...
    for (auto &pr : _cloned_server) {
        ...
        // [AUTO-TRANSLATED:d6a88e17]
        // Experiments have found that the cloneSocket method can save fd
        // resources, but the thread drift problem is more serious on some
        // systems
        pr.second->_socket->bindUdpSock(_socket->get_local_port(),
                                        _socket->get_local_ip());
#endif
    }
...
void UdpServer::cloneFrom(const UdpServer &that) {
    if (!that._socket) {
        throw std::invalid_argument(
            "UdpServer::cloneFrom other with null socket");
    }
    setupEvent();
    _cloned = true;
    // clone callbacks
...
static void emitSessionRecv(const SessionHelper::Ptr &helper,
                            const Buffer::Ptr &buf) {
    if (!helper->enable) {
...
void UdpServer::onRead_l(bool is_server_fd, const UdpServer::PeerIdType &id,
                         Buffer::Ptr &buf, sockaddr *addr, int addr_len) {
    // udp server fd收到数据时触发此函数；大部分情况下数据应该在peer
    // fd触发，此函数应该不是热点函数  [AUTO-TRANSLATED:f347ff20]
    // This function is triggered when the udp server fd receives data; in most
    // cases, the data should be triggered by the peer fd, and this function
    // should not be a hot spot
    bool is_new = false;
    if (auto helper = getOrCreateSession(id, buf, addr, addr_len, is_new)) {
        if (helper->session()->getPoller()->isCurrentThread()) {
            ...
            //[AUTO-TRANSLATED:1134538b] Since the socket read buffer is shared
            // and reused by all sockets on this thread, it cannot be used
            // across threads and must be transferred first
...
#if !defined(NDEBUG)
        if (!is_new) {
            TraceL << "UDP packet incoming from "
                   << (is_server_fd ? "server fd" : "other peer fd");
        }
#endif
    }
}
...
void UdpServer::onManagerSession() {
    ...
    auto lam = [copy_map]() {
        for (auto &pr : *copy_map) {
            ...
            try {
                ...
                WarnL << "Exception occurred when emit onManager: "
                      << ex.what();
            }
        }
    };
    if (_multi_poller) {
        EventPollerPool::Instance().for_each(
            [lam](const TaskExecutor::Ptr &executor) {
...
SessionHelper::Ptr UdpServer::getOrCreateSession(
    const UdpServer::PeerIdType &id, Buffer::Ptr &buf, sockaddr *addr,
    int addr_len, bool &is_new) {
    {
...
SessionHelper::Ptr UdpServer::createSession(const PeerIdType &id,
                                            Buffer::Ptr &buf,
    ...
    if (!socket) {
        ...
        //[AUTO-TRANSLATED:b218d68c] Socket creation failed, the data received
        // by this onRead event is discarded
        return nullptr;
    }
    ...
    std::weak_ptr<UdpServer> weak_self =
        std::static_pointer_cast<UdpServer>(shared_from_this());
    auto helper_creator = [this, weak_self, socket, addr_str,
                           id]() -> SessionHelper::Ptr {
        ...
        //[AUTO-TRANSLATED:c57a0d71] If the UdpSession class corresponding to
        // this client has already been created, return directly
        lock_guard<std::recursive_mutex> lck(*_session_mutex);
        auto it = _session_map->find(id);
        if (it != _session_map->end()) {
            return it->second;
        ...
        socket->bindPeerAddr((struct sockaddr *)addr_str.data(),
                             addr_str.size());
        ...
        std::weak_ptr<SessionHelper> weak_helper = helper;
        socket->setOnRead([weak_self, weak_helper, id](Buffer::Ptr &buf,
                                                       struct sockaddr *addr,
                                                       int addr_len) {
            ...
            //[AUTO-TRANSLATED:d5d147e4] Quickly determine if it's data for the
            // current session, usually should be true
            ...
            //[AUTO-TRANSLATED:e5f44445] Received data from a non-current peer
            // fd, let the server dispatch this data to the appropriate session
            // object
            strong_self->onRead_l(false, id, buf, addr, addr_len);
        });
        socket->setOnErr(
            [weak_self, weak_helper, id](const SockException &err) {
                // 在本函数作用域结束时移除会话对象  [AUTO-TRANSLATED:b2ade305]
                // Remove the session object when this function scope ends
                // 目的是确保移除会话前执行其 onError 函数
                // [AUTO-TRANSLATED:7d0329d7]
                // The purpose is to ensure the onError function is executed
                // before removing the session
                // 同时避免其 onError 函数抛异常时没有移除会话对象
                // [AUTO-TRANSLATED:354191bd]
                // And avoid not removing the session object when its onError
                ...
                onceToken token(nullptr, [&]() {
                    ...
                    // [AUTO-TRANSLATED:50dbd694]
                    // Delay removing the UDP session to prevent frequent and
                    // rapid object reconstruction
                    strong_self->_poller->doDelayTask(
                        kUdpDelayCloseMS, [weak_self, id]() {
                            if (auto strong_self = weak_self.lock()) {
                                // 从共享map中移除本session对象
                                // [AUTO-TRANSLATED:47ecbf11]
                                // Remove the current session object from the
                                // shared map
                                lock_guard<std::recursive_mutex> lck(
                                    *strong_self->_session_mutex);
                ...
                if (auto strong_helper = weak_helper.lock()) {
                    ...
                    TraceP(strong_helper->session())
                        << strong_helper->className() << " on err: " << err;
    ...
    if (socket->getPoller()->isCurrentThread()) {
        ...
        // This socket is allocated in this thread, directly create a helper
        // object
        return helper_creator();
    }

    // 该socket分配在其他线程，需要先转移走buffer，然后在其所在线程创建helper对象并处理数据
    // [AUTO-TRANSLATED:7816a13f]
    // This socket is allocated in another thread, need to transfer the buffer
    // first, then create a helper object in its thread and process the data
    auto cacheable_buf = std::move(buf);
    socket->getPoller()->async([helper_creator, cacheable_buf]() {
        ...
        if (helper) {
            ...
            // [AUTO-TRANSLATED:091f648e]
            // May not have actually created a helper object successfully, may
            // have obtained a helper object created by another thread
            helper->session()->getPoller()->async([helper, cacheable_buf]() {
                // 该数据不能丢弃，给session对象消费  [AUTO-TRANSLATED:6941e5fa]
                // This data cannot be discarded, provided to the session object
                // for consumption
