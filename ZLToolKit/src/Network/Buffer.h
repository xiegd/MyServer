...
class BufferOffset : public Buffer {
   ...
   private:
    ...
    template <typename T>
    static typename std::enable_if<::toolkit::is_pointer<T>::value,
                                   const T &>::type
        ...
        if (_data) {
            do {
                if (capacity > _capacity) {
                    //请求的内存大于当前内存，那么重新分配
                    //[AUTO-TRANSLATED:65306424] If the requested memory is
                    // greater than the current memory, reallocate
                    break;
                }

                if (_capacity < 2 * 1024) {
                    // 2K以下，不重复开辟内存，直接复用
                    // [AUTO-TRANSLATED:056416c0] Less than 2K, do not
                    // repeatedly allocate memory, reuse directly
                    return;
                }

                if (2 * capacity > _capacity) {
                    //如果请求的内存大于当前内存的一半，那么也复用
                    //[AUTO-TRANSLATED:c189d660] If the requested memory is
                    // greater than half of the current memory, also reuse
                    return;
                }
            } while (false);
