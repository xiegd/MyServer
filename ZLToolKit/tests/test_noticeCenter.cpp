...
int main() {
    ...
    // [AUTO-TRANSLATED:918a506c]
    // The First Parameter of the addListener Method is a Tag, Used to Delete
    // the Listener
    //需要注意的是监听回调的参数列表个数类型需要与emitEvent广播时的完全一致，否则会有无法预知的错误
    //[AUTO-TRANSLATED:b36668f5]
    // Note that the Number and Type of Parameters in the Listener Callback Must
    // be Exactly the Same as Those in the emitEvent Broadcast, Otherwise
    // Unpredictable Errors May Occur
    NoticeCenter::Instance().addListener(
        0, NOTICE_NAME1, [](int &a, const char *&b, double &c, string &d) {
            DebugL << a << " " << b << " " << c << " " << d;
    ...
    NoticeCenter::Instance().addListener(
        0, NOTICE_NAME2, [](string &d, double &c, const char *&b, int &a) {
            DebugL << a << " " << b << " " << c << " " << d;
    ...
    while (!g_bExitFlag) {
        ...
        //[AUTO-TRANSLATED:dc815907]
        // Broadcast the Event Every 1 Second, If the Parameter Type is
        // Uncertain, a Forced Conversion Can be Added
        NoticeCenter::Instance().emitEvent(NOTICE_NAME1, ++a, (const char *)"b",
                                           c, d);
